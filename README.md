...
2026-1 캡스톤 프로젝트. **기획과 디자인은 팀원과 함께**, **프론트엔드 구현과 Dify AI 에이전트 설계는 1인 개발**로 맡았습니다.
Claude Code를 활용해 구현하고, 아키텍처 설계와 검증은 직접 주도했습니다.

| 담당 | 산출물 |
|---|---|
| 프론트엔드 전체 | 프레임워크 없는 Vanilla JS · 자체 pub/sub 상태 관리자(`js/state.js`) · 유닛테스트 16개 |
| Dify AI 에이전트 설계 | [`dify/`](dify/) DSL 원본 (아래 §Dify 에이전트 아키텍처) |
| AI 게이트웨이 | [`gateway/`](gateway/) Cloudflare Worker — 키 은닉 · IP별 호출 한도 · 토큰/비용 로깅 |
| 품질 검증 | [`eval/`](eval/) 인텐트 분류 골든셋 평가 |
...
라이브 데모처럼 키를 배포에 포함할 수 없는 환경에서는 Cloudflare Worker 게이트웨이를 씁니다.

```
프론트(에이전트 이름만 전송) → Worker(/chat) → Dify API (키는 Worker secret)
```

키 은닉 · IP당 하루 40회 한도 · 요청별 토큰/비용 구조화 로깅 · 게이트웨이 실패 시 AI 카드만 비활성화하는 폴백을 제공합니다.
📄 **배포 절차와 설정 상세:** [`docs/gateway-setup.md`](docs/gateway-setup.md)
...
- **카드 소비분석 결과가 실제 예산 계산에 반영되지 않음** — 카드사 API 없이 목업 데이터로 "업로드 → AI 분석" 경험만 재현한 MVP 범위.
- **트레이너 챗봇이 정상 질문(class-2)을 무관 질문(class-6)으로 거절하는 오분류가 재현됨** — 2026-07-27 커밋(`5f84da3`)에서 프롬프트로 대응했으나, 수정 후 전체 골든셋 재검증 기록은 없음.
- **인텐트 분류 정확도 측정이 정상↔거절 경계만 판정** — class 1~5 사이의 세부 오분류는 잡지 못함.
- **인텐트 분류 골든셋(20문항 × 3회)을 게이트웨이 경유로 완주하지 못함** — Worker의 IP당 하루 40회 한도에 걸림.
- **Dify DSL export의 RAG 지식베이스 연결(`dataset_ids`)이 비어 있음** — 워크스페이스 ID 노출 방지를 위해 export 시 제거.

📄 **각 항목의 원인과 해결 방법 전문:** [`docs/limitations.md`](docs/limitations.md) · 평가 상세 [`eval/README.md`](eval/README.md)
